...
- `onEntityMoved(position, rotation)` — Called when this vibe's placed entity is moved/rotated in the editor. `position` = new world `{x,y,z}`, `rotation` = Euler degrees `{x,y,z}`; either may be null if unchanged. Optional hook; not defined on the base class.
  - _gotcha:_ Fires on transform-gizmo *finish* (gesture end), not per-frame during the drag. Children of `this.entity` follow it automatically — you only need this to re-anchor things you spawned separately (NPCs, detached sub-entities, physics bodies teleported by rigidbody). See `lamp.mjs` and `npc-character.mjs`.
- `onEditModeChanged(isEditing, context)` — Called when this vibe's in-app editor opens/closes for this entity (user selects/deselects it in the creator UI). `isEditing` = boolean; `context` = creator-badge state object or null. Optional hook; not defined on the base class.
  - _gotcha:_ Also fires once right after `initialize()` if the vibe loads while already selected, and with `false` when the entity is unloaded mid-edit. The same transitions are mirrored on the plugin event bus: `plugin:editModeChanged` (payload `{isEditing, entityId, context}`), `plugin:editModeEnter`, `plugin:editModeExit` — subscribe with `this.on(...)` if you prefer events.
...
- `createModel(url, options = {})` → `Promise<{entity: pc.Entity, asset: pc.Asset}>` — Instance-method wrapper around `ArrivalSpace.loadGLB(url, options)` that wires the loaded model back to THIS vibe: it parents the model to this plugin's entity, so the editor gizmo moves/rotates/scales it and it unloads with the vibe, and clicking it in edit mode selects this plugin's entity for editing. Prefer this over raw `loadGLB` when a plugin spawns its own GLB. Same options as loadGLB (parent, name, scale, position, rotation, castShadows, etc.); `position`/`rotation` are relative to this vibe's entity.
  - _gotcha:_ Raw `loadGLB` with no `parent` drops the model on the scene root — the gizmo then moves the vibe's entity while the visible mesh stays behind. Pass `parent` explicitly to `createModel` (a bone, a sub-container, `pc.app.root`) only when you want that detachment. It also injects an internal `_selectTarget` (this vibe's UserModelEntity) into options; edit-mode selection only wires up when a host UserModelEntity exists.
- `setOutline(entity = this.entity, on = true)` → `boolean` — Draw the platform's silhouette outline on an entity (the editor's hover highlight), switched on when the vibe decides rather than by the pointer. A boolean first arg is shorthand for this vibe's own entity: `this.setOutline(false)` clears it. Covers descendants with a `render` component; idempotent, safe from `update()`. `clearOutlines()` clears all of them. Also on `ArrivalSpace.setOutline(entity, on)`.
  - _gotcha:_ Prefer the instance method. The outline system refcounts outlined entities and detaches its post pass at zero, so an outline left on (or an entity destroyed while outlined) keeps that pass running for every visitor for the rest of the session; `this.setOutline` clears its own on destroy. Colour/thickness are app-wide, not per-entity.
