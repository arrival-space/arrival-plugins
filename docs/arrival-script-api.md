...
- `createModel(url, options = {})` → `Promise<{entity: pc.Entity, asset: pc.Asset}>` — Instance-method wrapper around `ArrivalSpace.loadGLB(url, options)` that also wires the loaded model back to THIS vibe: clicking it in edit mode selects this plugin's entity for editing. Prefer this over raw `loadGLB` when a plugin spawns its own GLB. Same options as loadGLB (parent, name, scale, position, rotation, castShadows, etc.).
  - _gotcha:_ It injects an internal `_selectTarget` (this vibe's UserModelEntity) into options; edit-mode selection only wires up when a host UserModelEntity exists. Otherwise identical to loadGLB.
